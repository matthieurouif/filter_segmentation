...
def knn_matte(img, trimap, mylambda=100):
    ...
    toc = time.time()
    print 'Finding nearest neighbors : %.1f s' % (toc-tic)
    sys.stdout.flush()
    ...
    H = 2*(L + mylambda*D)
    toc = time.time()
    print 'Computing sparse A        : %.1f s' % (toc-tic)
    sys.stdout.flush()
    ...
    toc = time.time()
    print 'Solving linear system     : %.1f s' % (toc-tic)
    sys.stdout.flush()
    return alpha
...
def main(model, input_path, output_path):
    ...
    print 'Image size                : ', mask0.shape
    sys.stdout.flush()

    # print 'Starting kNN matting'
    import time
    tic = time.time()
    alpha = knn_matte(img_rgb_small, trimap)
    toc = time.time()
    print 'Total processing time     : %.1f s' % (toc-tic)
...
if __name__ == '__main__':
    ...
    for input_path in image_path_list:
        ...
        # input_path = '../../../samples/woman_car.jpg'
